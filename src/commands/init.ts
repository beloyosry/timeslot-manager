...
program
    ...
    .action((options) => {
        try {
            ...
            console.log(chalk.blue("🚀 Initializing TimeSlot Manager..."));

            if (!fs.existsSync(prismaDir)) {
                console.log(
                    chalk.red(`❌ Prisma directory not found at: ${prismaDir}`)
                );
                console.log(
                    chalk.yellow('💡 Please run "npx prisma init" first')
                );
                process.exit(1);
            }
...
`;
            ...
            if (fs.existsSync(schemaPath)) {
                ...
                if (existingSchema.includes("model TimeSlot")) {
                    console.log(
                        chalk.yellow(
                            "⚠️  TimeSlot model already exists in your schema"
                        )
                    );
                ...
                console.log(
                    chalk.green("✅ TimeSlot model added to your schema")
                );
            } else {
                console.log(
                    chalk.red(`❌ schema.prisma not found at: ${schemaPath}`)
                );
                process.exit(1);
            }
